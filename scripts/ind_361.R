...
mydata <- data.frame(ind$Country.or.Area.Name,ind$Location,ind$Series.Code,
                     ind$X2002,ind$X2003,ind$X2004,
                     ind$X2005,ind$X2006,ind$X2007,ind$X2008,ind$X2009,
                     ind$X2010,ind$X2011,ind$X2012,ind$X2013)
mydata[is.na(mydata)] <- -999
colnames(mydata) <- c("pais","local","code",2002:2013)
mydata <- mydata[which(mydata$code=="SH_STA_TRAF"),]
